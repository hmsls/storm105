...
public class TridentTopologyApp {
	...
	public static void main(String[] args) throws Exception {
		...
		s = s.shuffle().each(new Fields("a","b"), new CheckEvenSumFilter()).parallelismHint(2);
		s = s.shuffle().each(new Fields("a","b"), new SumFunction(), new Fields("sum"));
		s = s.shuffle().each(new Fields("a","b","c","d","sum"), new AvgFunction(), new Fields("avg"));
